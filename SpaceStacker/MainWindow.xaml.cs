...
namespace SpaceStacker
{
    ...
    public partial class MainWindow : Window
    {
                ...
                if (input > 0)
                {
                    ...
                    if (existingDepartments < input)
                    {
                        ...
                        for (int i = 0; i < difference; i++)
                        {
                            ...
                            ExtraMethods.departmentExpanderGenerator(department, 4, functions,
            ...
            else
            {
                ...
                if (input > 0)
                {
                    ...
                    if (input > existingPrograms)
                    {
                        ...
                        ExtraMethods.AddProgram(programs, difference, existingPrograms, department, functions,
        ...
        void SelectedProgram_Chenged(object sender, EventArgs e)
        {
            ...
            // Update Cost Of The Program In The Dictionary
            this.boxesOfTheProject[programBoxName].cost = functions[selectedFunction]["cost"];

            // If Slider Changed Event Does Not Activate
            if ((keyRooms.Value == functions[selectedFunction]["keyVal"] && DGSF.Value == functions[selectedFunction]["DGSFVal"]))
            {
                // Calculating GSF And Cost Difference And Updating Values Of The Boxes Dictionary
                float GSF = this.boxesOfTheProject[programBoxName].boxTotalGSFValue;
            ...
            for (int i = programBoxVisualizationIndex; i < this.stackingVisualization.Children.Count; i++)
            {
                ...
                if (this.boxesOfTheProject[this.stackingVisualization.Children[i].GetName()].floor == programBoxFloor)
                {
                    ...
                    if (i == programBoxVisualizationIndex)
                    {
                        ...
                        float newRawProgramCost = newGSF * this.boxesOfTheProject[programBoxName].cost;
        ...
        private void CalculationsAndOutputs(float totalGSF, float totalRawDepartmentCost)
        {
            ...
            float circulationCost = (((float)this.CirculationSlider.Value) / 100) * totalGSF * functions["Circulation"]["cost"];

            float MEPCost = (((float)this.MEPSlider.Value) / 100) * totalGSF * functions["MEP"]["cost"];
        ...
        private void MaximizeProgramsInformationImage(object sender, RoutedEventArgs e)
        {
            ...
            foreach (Window objWindow in Application.Current.Windows)
            {
                ...
                if (aClassNameFromCollection == "ProgramsInformationSubWindow")
                {
                    if (objWindow.Visibility == Visibility.Visible)
                    {
                        isOpen = true;
